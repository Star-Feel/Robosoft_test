...
def find_contact_faces_idx_xy(faces_grid, x_min, y_min, grid_size,
                              position_collection):
    ...
    # 计算网格坐标（向量化计算）
    grid_coords = (element_position[0:2, :] -
                   np.array([[x_min], [y_min]])) / grid_size
    ...
    # 转换为数组输出
    return (np.array(position_idx_list,
                     dtype=int), np.array(face_idx_list,
                                          dtype=int), element_position)


def find_contact_faces_idx_xyz(faces_grid, x_min, y_min, z_min, grid_size,
                               position_collection):
    # 转换节点坐标为元素中心坐标
    element_position = elastica.contact_utils._node_to_element_position(
        position_collection)
    ...
    # 三维网格坐标计算（向量化）
    grid_coords = (element_position[:3, :] -
                   np.array([[x_min], [y_min], [z_min]])) / grid_size
    grid_x, grid_y, grid_z = np.round(grid_coords[0]), np.round(
        grid_coords[1]), np.round(grid_coords[2])
...
def surface_grid_numba_xy(faces, grid_size, face_x_left, face_x_right,
...
@numba.njit(cache=True)
def surface_grid_numba_xyz(faces, grid_size, face_x_left, face_x_right,
                           face_y_down, face_y_up, face_z_bottom, face_z_top):
    """
    三维网格划分版本
...
def surface_grid_xy(faces, grid_size):
    ...
    return dict(
        surface_grid_numba_xy(faces, grid_size, face_x_left, face_x_right,
                              face_y_down, face_y_up))
